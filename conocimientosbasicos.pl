...
clibro(matematicas_1,blanco,buho).
clibro(calculo_de_las_variables,azul,bat).
clibro(las_redes,blanco,lineas).
clibro(redes_cisco,rojo,lineasydatos).
clibro(computacional_intelligence,morado,cabezas).
clibro(introduccion_a_haskell,gris,cuadroslateral).
clibro(simply_logical,negro,lineas).
clibro(redes_wireless,azul,simbolowifi).
